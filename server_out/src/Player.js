...
export default class Player extends Sprite2D {
    ...
    handleMovementControls() {
        if (this.controls[Controls.UP])
            this.mY -= this.MOVEMENT_STRENGTH;
        if (this.controls[Controls.RIGHT])
            this.mX += this.MOVEMENT_STRENGTH;
        if (this.controls[Controls.DOWN])
            this.mY += this.MOVEMENT_STRENGTH;
        if (this.controls[Controls.LEFT])
            this.mX -= this.MOVEMENT_STRENGTH;
        if (this.controls[Controls.SHOOT])
            this.shoot();
    }
    /**
     * Moves the player.
    ...
    move() {
        ...
        if (Math.abs(this.mX) < 0.005)
            this.mX = 0;
        if (Math.abs(this.mY) < 0.005)
            this.mY = 0;
        ...
        if (Math.abs(this.mX) > this.MAX_VELOCITY)
            this.mX = Math.sign(this.mX) * this.MAX_VELOCITY;
        if (Math.abs(this.mY) > this.MAX_VELOCITY)
            this.mY = Math.sign(this.mY) * this.MAX_VELOCITY;
